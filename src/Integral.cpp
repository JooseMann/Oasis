...
auto Integral<Expression>::operator=(const Integral<Expression, Expression>& integral) const -> Integral
{
    std::unique_ptr<Expression> a = integral.GetLeastSigOp().Copy();
    std::unique_ptr<Expression> b = integral.GetMostSigOp().Copy();

    return Integral { *a, *b };
    // return *static_cast<Integral<Expression>*>(this);
}
